...
label_d = {'NetFluxlist':'Net Flux ($W/m^2$)', 'Tlist':'Surface Temperature (K)', "L":'Solar luminosity ($W/m^2$)'}
...
def make_stability_bifurcation(ODE_func, deriv='NetFlux', var='T', param_name='L', param_range=None, colormap=None,
    ...
    if stability is True:
        ax1.set_xlabel(label_d[var_name])
        ax1.set_ylabel(label_d[deriv_name])
    ...
    if bifurcation is True:
        if cols > 1:
            ...
            ax2.set_xlabel('{}/n{} {}'.format(label_d[param_name], 'Click to select', param_name))
            ...
            ax1.set_ylabel(label_d[var_name])
            ax1.set_xlim([minL, maxL])
            ax1.set_xlabel(label_d[param_name])
